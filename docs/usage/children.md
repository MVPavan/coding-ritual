...
Use a stable request key and retain the original request for recovery. An unrelated pending decision elsewhere under the same owner conservatively blocks replacement; let that decision settle first. Follow the returned current slot/generation; a superseded predecessor cannot advance the contractor. Integration retry and pending-landing recovery have separate meanings described in [contractor usage](contractor.md).
