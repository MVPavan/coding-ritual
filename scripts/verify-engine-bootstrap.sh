...
printf "Gate logs: %s\n" "$log_dir"
if uv run pytest -q tests/test_profiles_command.py tests/test_profiles_git_isolation.py tests/test_supervisor_sandbox.py tests/test_supervisor_sandbox_bound.py tests/test_codex_writer_qualification.py -m "not live" >"$log_dir/focused.log" 2>&1; then tail -n 2 "$log_dir/focused.log"; else cat "$log_dir/focused.log"; exit 1; fi
