...
Codex runs only through the **Codex CLI** directly (`codex exec` / `codex exec resume`) — never the codex-adapter plugin (uninstalled 2026-09-23). Drafts, plans, and completed diffs get their independent critique from a **spawned critic subagent** — a fresh agent, separate from whoever implemented; the user defines which model serves as critic. Findings come back numbered BLOCKER/MAJOR/MINOR with `file:line`, then a verdict. `small` tasks skip the critic unless risk is unusual. Detail: `.claude/project/tools.md`.
