...
from workflow_interpreter.bdio.wire import BoundSetting, Lifecycle, metadata_dict
...
def _reserve(
    ...
    refusal = bounds.steer_refusal(
        node=meta.node, round_no=meta.round_no, steer_closes=used, max_steers=limit or 0
    )
...
def record_state(
    ...
    with _guard(control.registration):
        return _record_state(client, reader, activation_id, control, state)
