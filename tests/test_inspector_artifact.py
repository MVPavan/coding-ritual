...
from tests._inspector import commit_all, crew_commit, make_git
...
def test_pin_artifact_records_identity_and_ref(worktree: Fixture) -> None:
    ...
    (tree / CREW_FILE).write_text("done\n", encoding="utf-8")
...
def test_in_repo_does_not_pin_a_commit_the_crew_did_not_declare(
    in_repo: Fixture,
) -> None:
...
def test_in_repo_with_no_effects_manifest_cannot_attribute_any_commit(
    ...
    separating the crew's commit from the human's, there is no attribution
    to be made at all.
    """
    in_repo.workspace.prepare(in_repo.activation, in_repo.node)
...
def test_pin_artifact_refuses_a_commit_that_does_not_descend_from_the_base(
    ...
    worktree.workspace.prepare(worktree.activation, worktree.node)
    (worktree.paths.worktree / CREW_FILE).write_text("work\n", encoding="utf-8")
...
def test_a_quarantined_commit_is_preserved_without_becoming_lineage(
    ...
    collects it — and `_is_crew_lineage` still refuses to move HEAD off it,
...
def test_a_commit_the_human_made_is_not_this_attempts_artifact(
    ...
    A dead crew's `$WF_EFFECTS_FILE` naming a path the human then commits in
    ...
    now be this activation's crew identity as well.
    """
    in_repo.workspace.prepare(in_repo.activation, in_repo.node)
    (in_repo.repo / CREW_FILE).write_text("the human's own work\n", encoding="utf-8")
    ...
    assert pin.identity is None
    assert pin.reason is not None
    assert "crew identity" in pin.reason


def test_a_commit_made_under_the_crew_identity_is_attributed(
    in_repo: Fixture,
) -> None:
    """The other side: the identity the wrapper stamps is what makes it ours.

    `CrewChannels.env()` puts `GIT_COMMITTER_*` on the child, so a commit the
    crew really made carries it and every §7.4 test passes together.
    """
    in_repo.workspace.prepare(in_repo.activation, in_repo.node)
    (in_repo.repo / CREW_FILE).write_text("crew output\n", encoding="utf-8")
    commit = crew_commit(in_repo.repo, "the attempt", in_repo.activation.activation_id)
...
def test_another_activations_crew_identity_does_not_attribute_here(
    in_repo: Fixture,
) -> None:
    ...
    """
    in_repo.workspace.prepare(in_repo.activation, in_repo.node)
    (in_repo.repo / CREW_FILE).write_text("an earlier attempt\n", encoding="utf-8")
    crew_commit(in_repo.repo, "the earlier attempt", STRANGER_ACTIVATION)
...
def test_the_worktree_mode_pin_asks_nothing_about_authorship(
    ...
    """
    worktree.workspace.prepare(worktree.activation, worktree.node)
    (worktree.paths.worktree / CREW_FILE).write_text("out\n", encoding="utf-8")
...
def test_the_crew_channels_stamp_this_activations_committer_identity() -> None:
    ...
    assert env[ENV_GIT_COMMITTER_EMAIL] == crew_committer_email("wf-42")
