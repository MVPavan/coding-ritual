...
def _contractor_lab(
    ...
    """A lab wired to run one contractor stage end to end on `store`."""
...
def test_a_stage_lands_end_to_end_on_each_switch_value(
    ...
    """
    lab = _contractor_lab(tmp_path, monkeypatch, signing_config, sign_payload, store)
...
def test_a_ledger_root_resumes_after_the_switch_goes_back_to_bd(
    ...
    assert _contractor_adapter(lab).record(STAGE).root_id == admitted.root_id


def _admit_only(lab: ForemanLab) -> ContractorRecord:
...
def test_the_locator_refuses_a_root_nothing_pinned(tmp_path: Path) -> None:
    """§3.2: no contractor record, no ledger row — a named refusal, never a guess.
...
def test_a_ledger_task_row_locates_every_root_of_a_run_with_no_contractor(
    tmp_path: Path,
) -> None:
...
def test_a_retry_creates_its_root_on_the_backend_its_record_names(
    ...
    """
    lab = _contractor_lab(tmp_path, monkeypatch, signing_config, sign_payload, first)
    adapter = _contractor_adapter(lab)
    admitted = _admit_only(lab)
...
def test_a_restarted_process_resumes_the_root_the_record_names(
    ...
    assert resumed.exit_code == 0, resumed.report
    assert _contractor_adapter(lab).record(STAGE).root_id == retried.root_id
...
@pytest.mark.acceptance
def test_a_contractor_task_keeps_its_worktree_until_the_export_is_pinned(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, signing_config, sign_payload
) -> None:
    """§3.9, D14: nothing is deleted before the task's record is durable.

    Settlement happens BEFORE the contractor lands and closes, so the tick that
...
def test_only_the_task_s_own_export_is_excused_from_coordinator_dirt() -> None:
    """§3.6: one path is the contractor's own write; everything else is somebody's work.
...
@pytest.mark.acceptance
def test_an_unrelated_file_under_wf_still_stops_the_contractor(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, signing_config, sign_payload
) -> None:
...
def test_a_checkout_sync_that_rewrites_the_export_is_refused(
    ...
    """
    from tests._inspector import commit_all, head_of
